...
// ============================================================
// RECORDATORIOS (SIN MONTOS - MEJORA DE LA NOTA)
// ============================================================
...
function renderizarRecordatorios() {
    ...
    DB.recordatorios.forEach(rec => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button class="btn btn-primary btn-sm" onclick="editarRecordatorio(${rec.ID})" title="✏️ Editar">✏️</button>
                <button class="btn btn-${rec.Completado ? 'warning' : 'success'} btn-sm" onclick="toggleRecordatorio(${rec.ID})" title="${rec.Completado ? '↩️ Reabrir' : '✅ Completar'}">
                    ${rec.Completado ? '↩️' : '✅'}
                </button>
                <button class="btn btn-danger btn-sm" onclick="eliminarRecordatorio(${rec.ID})" title="🗑️ Eliminar">️</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
// ============================================================
// NUEVO: CUENTAS POR COBRAR (MEJORA DE LA NOTA)
// ============================================================
function guardarCuentaPorCobrar() {
    ...
    if (!nombre) {
        alert('️ Debes ingresar el nombre del deudor');
        return;
    }
        ...
        if (!cuenta.Pagado && cuenta.SaldoPendiente > 0) {
            ...
            tr.innerHTML = `
                ...
                <td>
                    ...
                    <button class="btn btn-danger btn-sm" onclick="eliminarCuentaPorCobrar(${cuenta.ID})" title="🗑️ Eliminar">🗑️</button>
                </td>
            `;
            tbody.appendChild(tr);
        }
...
function confirmarPagoCobro() {
    ...
    if (!fechaCobro) {
        alert('⚠️ Debes seleccionar la fecha');
        return;
    }
...
// ============================================================
// DEUDAS / PAGAR (FUNCIONES ORIGINALES)
// ============================================================
...
function renderizarDeudas() {
    ...
    DB.deudas.forEach(deuda => {
        if (!deuda.Pagado && deuda.SaldoPendiente > 0) {
            ...
            if (deuda.Prioridad === 'Alta') {
                totalPrioridadAlta += deuda.SaldoPendiente;
            }
            ...
            tr.innerHTML = `
                ...
                <td>
                    <button class="btn btn-success btn-sm" onclick="abrirModalPago(${deuda.ID})" title="💵 Pagar">💵</button>
                    <button class="btn btn-primary btn-sm" onclick="editarDeuda(${deuda.ID})" title="️ Editar">✏️</button>
                    <button class="btn btn-info btn-sm" onclick="verHistorialPagos(${deuda.ID})" title="📜 Historial">📜</button>
                    <button class="btn btn-danger btn-sm" onclick="eliminarDeuda(${deuda.ID})" title="🗑️ Eliminar">️</button>
                </td>
            `;
            tbody.appendChild(tr);
        }
...
function verificarSaldoDisponible() {
    ...
    } else if (cuentaOrigen === 'Empresa_Bs') {
        saldoDisponible = DB.config.SaldoEmpresa_Bs || 0;
    } else if (cuentaOrigen === 'Personal') {
        saldoDisponible = DB.config.SaldoPersonal_USD || 0;
    } else if (cuentaOrigen === 'Personal_Bs') {
        saldoDisponible = DB.config.SaldoPersonal_Bs || 0;
    }
...
function confirmarPagoDeuda() {
    ...
    // Verificar saldo suficiente antes de pagar
    let saldoDisponible = 0;
    ...
    if (cuentaOrigen === 'Binance') {
        ...
        DB.config.SaldoEmpresa_Bs -= montoPago * DB.config.TasaBCV;
    } else if (cuentaOrigen === 'Personal') {
        DB.config.SaldoPersonal_USD -= montoPago;
    } else if (cuentaOrigen === 'Personal_Bs') {
        DB.config.SaldoPersonal_Bs -= montoPago * DB.config.TasaBCV;
    }
...
// ============================================================
// EMPLEADOS (FUNCIONES ORIGINALES)
// ============================================================
...
function renderizarNominaPagos() {
    ...
    nominaTemporal.forEach((emp, index) => {
        ...
        tr.innerHTML = `
            ...
            <td><button class="btn btn-danger btn-sm" onclick="eliminarDeNomina(${index})">🗑️</button></td>
        `;
        tbody.appendChild(tr);
        totalPagar += emp.Neto;
    });
...
function guardarNominaPagos() {
    ...
    // Verificar saldo suficiente antes de pagar nómina
    let saldoDisponible = 0;
    ...
    if (cuentaOrigen === 'Binance') {
        ...
        DB.config.SaldoEmpresa_Bs -= totalNomina * DB.config.TasaBCV;
    } else if (cuentaOrigen === 'Personal') {
        DB.config.SaldoPersonal_USD -= totalNomina;
    } else if (cuentaOrigen === 'Personal_Bs') {
        DB.config.SaldoPersonal_Bs -= totalNomina * DB.config.TasaBCV;
    }
...
// ============================================================
// BÚSQUEDA CON TARJETAS (MEJORA DE LA NOTA)
// ============================================================
function realizarBusqueda() {
    ...
    if (resultados.length === 0) {
        cont.innerHTML = '<p style="padding:20px;text-align:center;color:var(--text-secondary);">Sin resultados</p>';
    } else {
        cont.innerHTML = `<p style="padding:10px;font-weight:600; color:var(--text-primary);">${resultados.length} resultado(s)</p>` +
            resultados.map(r => {
                let botonesAccion = '';
                let claseCard = '';
                ...
                return `
                    <div style="padding:15px;background:var(--bg-input);margin:10px 0;border-radius:10px;border-left:5px solid var(--primary);box-shadow:0 2px 8px rgba(0,0,0,0.2); color:var(--text-primary);">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                            <span style="font-size:1.8em;">${r.icono}</span>
                            <div>
                                <strong style="font-size:1.1em;color:var(--text-primary);">${r.titulo}</strong>
                                <div style="color:var(--text-secondary);font-size:0.9em;">${r.tipo} - ${r.fecha}</div>
                            </div>
                        </div>
                        <div style="font-size:1.1em;color:var(--primary);font-weight:600;margin:5px 0;">${r.texto}</div>
                        ${r.detalle ? `<div style="color:var(--text-secondary);font-size:0.9em;margin-top:5px;">${r.detalle}</div>` : ''}
                        ${botonesAccion ? `<div style="margin-top:10px;">${botonesAccion}</div>` : ''}
                    </div>
                `;
            }).join('');
    }
...
// ============================================================
// DASHBOARD ACTUALIZADO (MEJORA DE LA NOTA)
// ============================================================
...
// ============================================================
// NUEVO: VER CIERRE POR FECHA (MEJORA DE LA NOTA)
// ============================================================
...
function generarPDF() {
    ...
    const tableData = DB.ingresos.map(ing => [
        ing.Fecha,
        ing.Total_USD.toFixed(2),
        ing.Total_Bs.toFixed(2),
        ing.SaldoFin_USD.toFixed(2),
        ing.SaldoFin_Bs.toFixed(2)
    ]);
...
function cambiarEstadoGS(estado, texto) {
    ...
    if (estado === 'conectado') {
        ...
        GS_CONECTADO = true;
        const btnDesc = document.getElementById('btnDesconectarGS');
...
function conectarGoogleSheets() {
    ...
    if (!url) {
        alert('⚠️ Pega la URL de tu Apps Script');
        return;
    }
