    ...
    if (resultados.length === 0) {
        ...
        cont.innerHTML = `<p class="busq-resultados-contador">${resultados.length} resultado(s)</p>` +
            resultados.map(r => {
                ...
                return `<div class="busq-card"><div class="busq-card-header"><span class="busq-card-icono">${r.icono}</span><div><strong class="busq-card-titulo">${r.titulo}</strong><div class="busq-card-sub">${r.tipo} - ${r.fecha}</div></div></div><div class="busq-card-monto">${r.texto}</div>${r.detalle ? `<div class="busq-card-detalle">${r.detalle}</div>` : ''}${botonesAccion ? `<div class="busq-card-acciones">${botonesAccion}</div>` : ''}</div>`;
            }).join('');
    }
}
    ...
    grid.innerHTML = datos.map(p => `
        <div class="prov-card">
            <div class="prov-card-header">
                ...
                <div>
                    <button class="btn btn-secondary btn-sm" onclick="abrirModalContactoProveedor(${p.id})" title="Editar">✏️</button>
                    <button class="btn btn-danger btn-sm" onclick="eliminarContactoProveedor(${p.id})" title="Eliminar">🗑️</button>
                </div>
            </div>
            <div class="prov-card-info">📞 ${escapeHtmlProv(p.numero)}</div>
            ${p.etiqueta ? `<div class="prov-tags">${p.etiqueta.split(',').map(t => `<span class="prov-tag">${escapeHtmlProv(t.trim())}</span>`).join('')}</div>` : ''}
            ${p.descripcion ? `<div class="prov-card-info">📝 ${escapeHtmlProv(p.descripcion)}</div>` : ''}
            <div class="prov-card-actions">
                <button class="btn btn-success btn-sm" onclick="abrirWhatsAppContacto('${escapeHtmlProv(p.numero)}', '${escapeHtmlProv(p.nombre)}')">💬 WhatsApp</button>
                <button class="btn btn-primary btn-sm" onclick="abrirLlamadaContacto('${escapeHtmlProv(p.numero)}')">📞 Llamar</button>
            </div>
        </div>
    `).join('');
}
