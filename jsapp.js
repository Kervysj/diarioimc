...
// NUEVO: muestra en vivo el equivalente en Bs cuando se cobra hacia una cuenta en Bs
function actualizarEquivalenciaCobro() {
    ...
    if (!montoCobroEl || !cuentaDestinoEl || !elEquivalencia) return;
    const montoCobro = parseFloat(montoCobroEl.value) || 0;
    const cuentaDestino = cuentaDestinoEl.value;
    const tasa = DB.config.TasaBCV || 0;
    if (cuentaDestino === 'Empresa_Bs') {
        elEquivalencia.style.display = 'block';
        elEquivalencia.textContent = '🔄 Equivale a Bs. ' + (montoCobro * tasa).toFixed(2) + ' (Tasa BCV: ' + tasa.toFixed(2) + ')';
    } else {
