...
let DB = {
    ...
    proveedoresContacto: [],
    notas: '',
    saldosInicialesBloqueados: false,
    cierreCajaBloqueado: false
};
...
// Clave maestra: requerida para el reseteo total del sistema y para eliminar
// proveedores del directorio de contactos.
const CLAVE_ADMIN = '21175383';
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    renderizarContactosProveedores();
...
// ============================================================
// NUEVO: ACTUALIZAR TASA BCV SIN NECESIDAD DE CERRAR CAJA
// ============================================================
// Permite actualizar la Tasa BCV en cualquier momento (por ejemplo antes de
// pagar o cobrar) sin depender de que se haya cerrado la caja del día.
function editarTasaBCV() {
    const tasaActual = DB.config.TasaBCV || 0;
    const nuevaTasaStr = prompt('Ingresa la Tasa BCV actual:', tasaActual > 0 ? tasaActual : '');
    if (nuevaTasaStr === null) return;
    const nuevaTasa = parseFloat(nuevaTasaStr);
    if (isNaN(nuevaTasa) || nuevaTasa <= 0) { alert('⚠️ Ingresa una tasa válida mayor a 0'); return; }
    DB.config.TasaBCV = nuevaTasa;
    const ingresoTasaEl = document.getElementById('ingresoTasa');
    if (ingresoTasaEl) ingresoTasaEl.value = nuevaTasa;
    guardarEnLocalStorage();
    actualizarContexto();
    mostrarIndicadorGuardado();
    alert('✅ Tasa BCV actualizada a Bs. ' + nuevaTasa.toFixed(2));
}
...
function bloquearCamposSaldos() {
    const campos = ['saldoIniUSD', 'saldoIniBS'];
    campos.forEach(id => {
...
function calcularTotales() {
    ...
    const totalUSD = efectivoUSD + zelle + binance;
    
    const efectivoBS = parseFloat(document.getElementById('efectivoBS').value) || 0;
    const biopago1 = parseFloat(document.getElementById('biopago1').value) || 0;
    const biopago2 = parseFloat(document.getElementById('biopago2').value) || 0;
    const biopago3 = parseFloat(document.getElementById('biopago3').value) || 0;
    const transferencia = parseFloat(document.getElementById('transferencia').value) || 0;
    const punto1 = parseFloat(document.getElementById('punto1').value) || 0;
    const punto2 = parseFloat(document.getElementById('punto2').value) || 0;
    const punto3 = parseFloat(document.getElementById('punto3').value) || 0;
    const totalBS = efectivoBS + biopago1 + biopago2 + biopago3 + transferencia + punto1 + punto2 + punto3;
...
function guardarIngreso() {
    ...
    calcularTotales(); // Respaldo: asegura que el total en pantalla esté sincronizado antes de guardar
    ...
    const ingreso = {
        ...
        Biopago1_Bs: parseFloat(document.getElementById('biopago1').value) || 0,
        Biopago2_Bs: parseFloat(document.getElementById('biopago2').value) || 0,
        Biopago3_Bs: parseFloat(document.getElementById('biopago3').value) || 0,
        ...
        Punto1_Bs: parseFloat(document.getElementById('punto1').value) || 0,
        Punto2_Bs: parseFloat(document.getElementById('punto2').value) || 0,
        Punto3_Bs: parseFloat(document.getElementById('punto3').value) || 0,
        Observacion: document.getElementById('observacion').value
    };
    
    ingreso.Total_USD = ingreso.Efectivo_USD + ingreso.Zelle_USD + ingreso.Binance_USD;
    
    ingreso.Total_Bs = ingreso.Efectivo_Bs + ingreso.Biopago1_Bs + ingreso.Biopago2_Bs + ingreso.Biopago3_Bs +
        ingreso.Transferencia_Bs + ingreso.Punto1_Bs + ingreso.Punto2_Bs + ingreso.Punto3_Bs;
    ...
    DB.config.SaldoEmpresa_USD += ingreso.Efectivo_USD;
    DB.config.SaldoEmpresa_Bs += ingreso.Efectivo_Bs + ingreso.Biopago1_Bs + ingreso.Biopago2_Bs + ingreso.Biopago3_Bs +
        ingreso.Transferencia_Bs + ingreso.Punto1_Bs + ingreso.Punto2_Bs + ingreso.Punto3_Bs;
...
function renderizarIngresos() {
    ...
    DB.ingresos.forEach(ing => {
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${ing.Fecha}</td><td>${ing.TasaBCV}</td><td>${ing.SaldoIni_USD.toFixed(2)}</td><td>${ing.SaldoIni_Bs.toFixed(2)}</td><td>${(ing.Efectivo_Bs||0).toFixed(2)}</td><td>${(ing.Efectivo_USD||0).toFixed(2)}</td><td>${(ing.Biopago1_Bs||0).toFixed(2)}</td><td>${(ing.Biopago2_Bs||0).toFixed(2)}</td><td>${(ing.Biopago3_Bs||0).toFixed(2)}</td><td>${(ing.Transferencia_Bs||0).toFixed(2)}</td><td>${(ing.Zelle_USD||0).toFixed(2)}</td><td>${(ing.Binance_USD||0).toFixed(2)}</td><td>${(ing.Punto1_Bs||0).toFixed(2)}</td><td>${(ing.Punto2_Bs||0).toFixed(2)}</td><td>${(ing.Punto3_Bs||0).toFixed(2)}</td><td><strong>${ing.Total_USD.toFixed(2)}</strong></td><td><strong>${ing.Total_Bs.toFixed(2)}</strong></td><td>${ing.SaldoFin_USD.toFixed(2)}</td><td>${ing.SaldoFin_Bs.toFixed(2)}</td><td><button class="btn btn-primary btn-sm" onclick="editarIngreso(${ing.ID})">✏️</button> <button class="btn btn-danger btn-sm" onclick="eliminarIngreso(${ing.ID})">🗑️</button></td>`;
        tbody.appendChild(tr);
    });
}
...
function editarIngreso(id) {
    ...
    document.getElementById('editIngresoEfectivoUSD').value = ing.Efectivo_USD || 0;
    document.getElementById('editIngresoEfectivoBS').value = ing.Efectivo_Bs || 0;
    document.getElementById('editIngresoBiopago1').value = ing.Biopago1_Bs || 0;
    document.getElementById('editIngresoBiopago2').value = ing.Biopago2_Bs || 0;
    document.getElementById('editIngresoBiopago3').value = ing.Biopago3_Bs || 0;
    document.getElementById('editIngresoTransferencia').value = ing.Transferencia_Bs || 0;
    document.getElementById('editIngresoZelle').value = ing.Zelle_USD || 0;
    document.getElementById('editIngresoBinance').value = ing.Binance_USD || 0;
    document.getElementById('editIngresoPunto1').value = ing.Punto1_Bs || 0;
    document.getElementById('editIngresoPunto2').value = ing.Punto2_Bs || 0;
    document.getElementById('editIngresoPunto3').value = ing.Punto3_Bs || 0;
...
function guardarEdicionIngreso() {
    ...
    ing.Biopago1_Bs = parseFloat(document.getElementById('editIngresoBiopago1').value) || 0;
    ing.Biopago2_Bs = parseFloat(document.getElementById('editIngresoBiopago2').value) || 0;
    ing.Biopago3_Bs = parseFloat(document.getElementById('editIngresoBiopago3').value) || 0;
    ...
    ing.Punto1_Bs = parseFloat(document.getElementById('editIngresoPunto1').value) || 0;
    ing.Punto2_Bs = parseFloat(document.getElementById('editIngresoPunto2').value) || 0;
    ing.Punto3_Bs = parseFloat(document.getElementById('editIngresoPunto3').value) || 0;
    ing.Observacion = document.getElementById('editIngresoObservacion').value;
    
    ing.Total_USD = ing.Efectivo_USD + ing.Zelle_USD + ing.Binance_USD;
    ing.Total_Bs = ing.Efectivo_Bs + ing.Biopago1_Bs + ing.Biopago2_Bs + ing.Biopago3_Bs +
        ing.Transferencia_Bs + ing.Punto1_Bs + ing.Punto2_Bs + ing.Punto3_Bs;
...
function limpiarIngreso() {
    ...
    document.getElementById('biopago1').value = 0;
    document.getElementById('biopago2').value = 0;
    document.getElementById('biopago3').value = 0;
    ...
    document.getElementById('punto1').value = 0;
    document.getElementById('punto2').value = 0;
    document.getElementById('punto3').value = 0;
...
// Unidad actual del campo "Monto a Recibir" del modal de cobro: 'USD' o 'BS'.
let _cobroUnidad = 'USD';
...
function abrirModalCobro(id) {
    ...
    document.getElementById('pagoCobrarCuenta').value = 'Empresa';
    _cobroUnidad = 'USD';
    const labelMonto = document.getElementById('pagoCobrarMontoLabel');
    if (labelMonto) labelMonto.textContent = '💵 Monto a Recibir ($)';
    ...
    const modalSaldoEmpresaCobro = document.getElementById('modalSaldoEmpresaCobro');
    if (modalSaldoEmpresaCobro) modalSaldoEmpresaCobro.textContent = '$' + (DB.config.SaldoEmpresa_USD || 0).toFixed(2);
    const modalSaldoEmpresaBsCobro = document.getElementById('modalSaldoEmpresaBsCobro');
    if (modalSaldoEmpresaBsCobro) modalSaldoEmpresaBsCobro.textContent = 'Bs. ' + (DB.config.SaldoEmpresa_Bs || 0).toFixed(2);
    const modalSaldoBinanceCobro = document.getElementById('modalSaldoBinanceCobro');
    if (modalSaldoBinanceCobro) modalSaldoBinanceCobro.textContent = '$' + (DB.config.SaldoBinance_USD || 0).toFixed(2);
    const modalSaldoZelleCobro = document.getElementById('modalSaldoZelleCobro');
    if (modalSaldoZelleCobro) modalSaldoZelleCobro.textContent = '$' + (DB.config.SaldoZelle_USD || 0).toFixed(2);
    document.getElementById('modalPagarCobrar').style.display = 'flex';
    actualizarEquivalenciaCobro();
}

// NUEVO: al cambiar la cuenta destino, si pasa a ser la cuenta en Bs, convierte
// automáticamente el monto actual de $ a Bs (sugiriendo el equivalente exacto
// que debería recibir); si vuelve a una cuenta en $, convierte de Bs a $.
function cambiarUnidadCobro() {
    const cuentaDestinoEl = document.getElementById('pagoCobrarCuenta');
    const montoEl = document.getElementById('pagoCobrarMonto');
    const labelMonto = document.getElementById('pagoCobrarMontoLabel');
    if (!cuentaDestinoEl || !montoEl) return;
    const nuevaEsBs = cuentaDestinoEl.value === 'Empresa_Bs';
    const tasa = DB.config.TasaBCV || 0;
    const valorActual = parseFloat(montoEl.value) || 0;
    if (nuevaEsBs && _cobroUnidad === 'USD') {
        montoEl.value = (valorActual * tasa).toFixed(2);
        _cobroUnidad = 'BS';
        if (labelMonto) labelMonto.textContent = '💴 Monto a Recibir (Bs)';
    } else if (!nuevaEsBs && _cobroUnidad === 'BS') {
        montoEl.value = tasa > 0 ? (valorActual / tasa).toFixed(2) : '0.00';
        _cobroUnidad = 'USD';
        if (labelMonto) labelMonto.textContent = '💵 Monto a Recibir ($)';
    }
}

// NUEVO: muestra en vivo, solo de forma visual e informativa, a cuánto
// equivale en la otra moneda el monto que se está por cobrar. Si se recibe en
// Bs (posiblemente un monto distinto al exacto, por ejemplo si pagan menos),
// muestra cuánto equivale en $; el cálculo real de la deuda se hace con ese
// mismo equivalente.
function actualizarEquivalenciaCobro() {
    cambiarUnidadCobro();
    const montoCobroEl = document.getElementById('pagoCobrarMonto');
    const elEquivalencia = document.getElementById('cobroEquivalenciaBs');
    if (!montoCobroEl || !elEquivalencia) return;
    const monto = parseFloat(montoCobroEl.value) || 0;
    const tasa = DB.config.TasaBCV || 0;
    if (_cobroUnidad === 'BS') {
        elEquivalencia.style.display = 'block';
        if (tasa > 0) {
            elEquivalencia.textContent = '🔄 Equivale a $' + (monto / tasa).toFixed(2) + ' (Tasa BCV: ' + tasa.toFixed(2) + ')';
        } else {
            elEquivalencia.textContent = '⚠️ Actualiza la Tasa BCV (arriba, en la barra superior) para calcular el equivalente en $';
        }
    } else {
        elEquivalencia.style.display = 'none';
    }
}
...
function confirmarPagoCobro() {
    ...
    const montoIngresado = parseFloat(document.getElementById('pagoCobrarMonto').value) || 0;
    ...
    const tasa = DB.config.TasaBCV || 0;
    const esBs = cuentaDestino === 'Empresa_Bs';
    
    if (montoIngresado <= 0) { alert('⚠️ Debes ingresar un monto mayor a 0'); return; }
    if (esBs && tasa <= 0) { alert('⚠️ Debes actualizar la Tasa BCV antes de cobrar en bolívares'); return; }
    
    // NUEVO: si la cuenta destino es en Bs, el monto ingresado ya está en Bs
    // (puede ser distinto al equivalente exacto, por ejemplo si pagan menos).
    // Se convierte a $ para saber cuánto de la deuda (que está en $) se cancela.
    const montoCobro = esBs ? (montoIngresado / tasa) : montoIngresado;
    
    if (montoCobro > cuenta.SaldoPendiente + 0.01) { alert('⚠️ El monto no puede ser mayor al saldo pendiente'); return; }
    
    if (cuentaDestino === 'Binance') DB.config.SaldoBinance_USD += montoIngresado;
    else if (cuentaDestino === 'Zelle') DB.config.SaldoZelle_USD += montoIngresado;
    else if (cuentaDestino === 'Empresa') DB.config.SaldoEmpresa_USD += montoIngresado;
    else if (cuentaDestino === 'Empresa_Bs') DB.config.SaldoEmpresa_Bs += montoIngresado; // ya está en Bs
    
    const cobroRegistro = { Fecha: fechaCobro, MontoUSD: montoCobro, MontoBs: esBs ? montoIngresado : montoCobro * tasa, CuentaDestino: cuentaDestino, Referencia: referencia };
...
function verificarSaldoDisponible() {
    ...
    const esCuentaBs = cuentaOrigen.endsWith('_Bs');
    const tasa = DB.config.TasaBCV || 0;
    ...
    const elEquivalencia = document.getElementById('pagoDeudaEquivalenciaBs');
    if (elEquivalencia) {
        if (esCuentaBs) {
            elEquivalencia.style.display = 'block';
            elEquivalencia.textContent = '🔄 Equivale a Bs. ' + (montoPago * tasa).toFixed(2) + ' (Tasa BCV: ' + tasa.toFixed(2) + ')';
        } else {
            elEquivalencia.style.display = 'none';
        }
    }
    // NUEVO: si la cuenta es en Bs y no hay Tasa BCV actualizada, no se puede
    // calcular el equivalente; se avisa para actualizar la tasa primero.
    if (esCuentaBs && tasa <= 0) {
        alerta.innerHTML = '⚠️ <strong>Actualiza la Tasa BCV</strong> (barra superior) antes de pagar con esta cuenta.';
        alerta.style.display = 'block';
        return;
    }
    // NUEVO: si la cuenta es en Bs, el monto (siempre ingresado en $) se debe
    // convertir a Bs antes de compararlo con el saldo disponible (que está en Bs).
    const montoComparar = esCuentaBs ? (montoPago * tasa) : montoPago;
    alerta.innerHTML = '⚠️ <strong>Saldo insuficiente</strong> en la cuenta seleccionada.';
    alerta.style.display = montoComparar > saldoDisponible ? 'block' : 'none';
}
...
function confirmarPagoDeuda() {
    ...
    let saldoDisponible = 0;
    const esCuentaOrigenBs = cuentaOrigen.endsWith('_Bs');
    const tasaPago = DB.config.TasaBCV || 0;
    if (cuentaOrigen === 'Binance') saldoDisponible = DB.config.SaldoBinance_USD || 0;
    else if (cuentaOrigen === 'Zelle') saldoDisponible = DB.config.SaldoZelle_USD || 0;
    else if (cuentaOrigen === 'Empresa') saldoDisponible = DB.config.SaldoEmpresa_USD || 0;
    else if (cuentaOrigen === 'Empresa_Bs') saldoDisponible = DB.config.SaldoEmpresa_Bs || 0;
    
    // NUEVO: si es cuenta en Bs y no hay Tasa BCV actualizada, no se deja pagar.
    if (esCuentaOrigenBs && tasaPago <= 0) { alert('⚠️ Debes actualizar la Tasa BCV (barra superior) antes de pagar con esta cuenta'); return; }
    
    // NUEVO: comparar en la misma moneda de la cuenta seleccionada (el monto
    // siempre se ingresa en $ y aquí se convierte a Bs si la cuenta es en Bs).
    const montoPagoComparar = esCuentaOrigenBs ? (montoPago * tasaPago) : montoPago;
    if (montoPagoComparar > saldoDisponible) { alert('⚠️ Saldo insuficiente. Disponible: ' + (esCuentaOrigenBs ? 'Bs. ' + saldoDisponible.toFixed(2) : '$' + saldoDisponible.toFixed(2))); return; }
...
function abrirModalPagoNomina(id) {
    ...
    document.getElementById('nominaTasaEditable').oninput = function() {
        ...
        actualizarSaldoDisponibleNomina();
    };
    
    actualizarSaldoDisponibleNomina();
}

// NUEVO: Muestra el saldo disponible en la cuenta seleccionada y bloquea el
// pago si no alcanza, tanto en cuentas en USD como en cuentas en Bs.
function actualizarSaldoDisponibleNomina() {
    ...
    const esCuentaBs = cuentaOrigen === 'Empresa_Bs';
    ...
    const montoRequerido = esCuentaBs ? (emp.SueldoUSD * tasaUsada) : emp.SueldoUSD;
    
    document.getElementById('nominaSaldoDisponibleTexto').textContent =
        esCuentaBs ? ('Bs. ' + saldoDisponible.toFixed(2)) : ('$' + saldoDisponible.toFixed(2));
    
    const alcanza = montoRequerido <= saldoDisponible;
    const alerta = document.getElementById('alertaSaldoNomina');
    const btnConfirmar = document.getElementById('btnConfirmarPagoNomina');
    if (alerta) alerta.style.display = alcanza ? 'none' : 'block';
    if (btnConfirmar) btnConfirmar.disabled = !alcanza;
...
function confirmarPagoNominaIndividual() {
    ...
    // Verifica el saldo en la MONEDA de la cuenta seleccionada (Bs o USD según corresponda)
    if (!actualizarSaldoDisponibleNomina()) {
        alert('⚠️ Saldo insuficiente en la cuenta seleccionada. No puedes proceder a pagar.');
        return;
    }
...
// ============================================================
// NUEVO: Filtros inteligentes tipo tarjeta para la Búsqueda
function seleccionarFiltroBusqueda(filtro) {
    document.getElementById('busqFiltro').value = filtro;
    document.querySelectorAll('#busqFiltroTarjetas .busq-chip').forEach(chip => {
        chip.classList.toggle('active', chip.dataset.filtro === filtro);
    });
    realizarBusqueda();
}

// NUEVO: Rangos de fecha rápidos para la Búsqueda
function aplicarRangoRapidoBusqueda(rango) {
    const hoy = new Date();
    const iso = (d) => d.toISOString().split('T')[0];
    let desde = '', hasta = '';
    
    if (rango === 'hoy') {
        desde = iso(hoy); hasta = iso(hoy);
    } else if (rango === 'semana') {
        const diaSemana = hoy.getDay();
        const inicio = new Date(hoy);
        inicio.setDate(hoy.getDate() - diaSemana);
        desde = iso(inicio); hasta = iso(hoy);
    } else if (rango === 'mes') {
    ...
    document.getElementById('busqDesde').value = desde;
    document.getElementById('busqHasta').value = hasta;
    realizarBusqueda();
}

// NUEVO: Arma el desglose de métodos de pago de un cierre de caja (solo los que tienen monto)
function desgloseIngresoDia(ing) {
    const partes = [];
    if (ing.Efectivo_Bs) partes.push(`Efectivo: Bs. ${ing.Efectivo_Bs.toFixed(2)}`);
    if (ing.Efectivo_USD) partes.push(`Divisas: $${ing.Efectivo_USD.toFixed(2)}`);
    if (ing.Biopago1_Bs) partes.push(`Biopago 1: Bs. ${ing.Biopago1_Bs.toFixed(2)}`);
    if (ing.Biopago2_Bs) partes.push(`Biopago 2: Bs. ${ing.Biopago2_Bs.toFixed(2)}`);
    if (ing.Biopago3_Bs) partes.push(`Biopago 3: Bs. ${ing.Biopago3_Bs.toFixed(2)}`);
    if (ing.Transferencia_Bs) partes.push(`Transferencia: Bs. ${ing.Transferencia_Bs.toFixed(2)}`);
    if (ing.Binance_USD) partes.push(`Binance: $${ing.Binance_USD.toFixed(2)}`);
    if (ing.Zelle_USD) partes.push(`Zelle: $${ing.Zelle_USD.toFixed(2)}`);
    if (ing.Punto1_Bs) partes.push(`Punto 1: Bs. ${ing.Punto1_Bs.toFixed(2)}`);
    if (ing.Punto2_Bs) partes.push(`Punto 2: Bs. ${ing.Punto2_Bs.toFixed(2)}`);
    if (ing.Punto3_Bs) partes.push(`Punto 3: Bs. ${ing.Punto3_Bs.toFixed(2)}`);
    return partes.join(' • ');
}
...
function realizarBusqueda() {
    ...
    if (filtro === 'todos' || filtro === 'ingresos') {
        DB.ingresos.forEach(ing => {
            ...
            // Cuadre de Caja: solo se muestra lo que entró ESE día (sin saldo inicial/final ni tasa)
            resultados.push({ tipo: 'Ingreso', fecha: ing.Fecha, icono: '💵', titulo: `Cuadre de Caja - ${ing.Fecha}`, texto: `Entró: $${ing.Total_USD.toFixed(2)} / Bs. ${ing.Total_Bs.toFixed(2)}`, detalle: desgloseIngresoDia(ing), id: ing.ID, data: ing });
        });
    }
    ...
    if (resultados.length === 0) {
        cont.innerHTML = '<p class="busq-sin-resultados">Sin resultados</p>';
    } else {
        cont.innerHTML = `<p class="busq-resultados-contador">${resultados.length} resultado(s)</p>` +
            resultados.map(r => {
                let botonesAccion = '';
                ...
                return `<div class="busq-card" data-tipo="${r.tipo}"><div class="busq-card-top"><span class="busq-card-icono">${r.icono}</span><div><span class="busq-card-badge">${r.tipo}</span><strong class="busq-card-titulo">${r.titulo}</strong><div class="busq-card-sub">${r.fecha}</div></div></div><div class="busq-card-body"><div class="busq-card-monto">${r.texto}</div>${r.detalle ? `<div class="busq-card-detalle">${r.detalle}</div>` : ''}</div>${botonesAccion ? `<div class="busq-card-acciones">${botonesAccion}</div>` : ''}</div>`;
            }).join('');
    }
}
...
function confirmarReset() {
    ...
    DB = { config: { TasaBCV: 0, SaldoBinance_USD: 0, SaldoZelle_USD: 0, SaldoEmpresa_USD: 0, SaldoEmpresa_Bs: 0, NombreNegocio: 'Mi Negocio' }, ingresos: [], recordatorios: [], deudas: [], cuentasPorCobrar: [], empleados: [], nominaPagos: [], resumenDiario: [], proveedoresContacto: [], notas: '', saldosInicialesBloqueados: false, cierreCajaBloqueado: false };
    ...
    actualizarDashboard(); actualizarListaProveedores(); actualizarBloqueCierre(); recalcularResumenDiario(); actualizarPanelSaldos(); actualizarContexto(); renderizarContactosProveedores();
...
function cargarDesdeLocalStorage() {
    ...
    if (saved) {
        ...
        if (!DB.proveedoresContacto) DB.proveedoresContacto = [];
        ...
        renderizarIngresos(); renderizarRecordatorios(); renderizarDeudas(); renderizarCobrar(); renderizarEmpleados(); renderizarHistorialNomina();
        recalcularResumenDiario(); actualizarBloqueCierre(); actualizarDashboard(); actualizarListaProveedores(); renderizarContactosProveedores();
...
function cargarExcel(event) {
    ...
    reader.onload = (e) => {
        ...
        DB.proveedoresContacto = leerHoja(wb, 'ProveedoresContacto') || [];
        DB.resumenDiario = [];
        if (!DB.config.SaldoZelle_USD) DB.config.SaldoZelle_USD = 0;
        document.getElementById('ingresoTasa').value = DB.config.TasaBCV;
        guardarEnLocalStorage();
        renderizarIngresos(); renderizarRecordatorios(); renderizarDeudas(); renderizarCobrar(); renderizarEmpleados(); renderizarHistorialNomina();
        recalcularResumenDiario(); actualizarBloqueCierre(); actualizarDashboard(); actualizarListaProveedores(); renderizarContactosProveedores(); actualizarPanelSaldos(); actualizarContexto();
...
function guardarExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(DB.proveedoresContacto || []), 'ProveedoresContacto');
    const nombre = (DB.config.NombreNegocio || 'FinanzasPro').replace(/\s+/g,'_');
...
async function cargarDesdeSheets() { if (!GS_URL) { alert('️ Primero conecta tu Google Sheet'); return; } cambiarEstadoGS('cargando', 'Cargando datos...'); try { const res = await fetch(GS_URL + '?action=load'); const data = await res.json(); if (data.success) { DB = data.data; if (!DB.resumenDiario) DB.resumenDiario = []; if (!DB.cuentasPorCobrar) DB.cuentasPorCobrar = []; if (!DB.proveedoresContacto) DB.proveedoresContacto = []; if (!DB.config.SaldoZelle_USD) DB.config.SaldoZelle_USD = 0; if (DB.config) document.getElementById('ingresoTasa').value = DB.config.TasaBCV || 0; guardarEnLocalStorage(); renderizarIngresos(); renderizarRecordatorios(); renderizarDeudas(); renderizarCobrar(); renderizarEmpleados(); renderizarHistorialNomina(); recalcularResumenDiario(); actualizarBloqueCierre(); actualizarDashboard(); actualizarListaProveedores(); renderizarContactosProveedores(); actualizarPanelSaldos(); actualizarContexto(); cambiarEstadoGS('conectado'); alert('✅ Datos cargados desde Google Sheets'); } else { cambiarEstadoGS('error', data.error || 'Error al cargar'); } } catch (err) { cambiarEstadoGS('error', err.message); alert('❌ Error: ' + err.message); } }
...
function generarGuiaGoogleSheets() {
    const codigoScript = `function doGet(e) {
...
function handleRequest(e) {
  ...
  try {
    ...
    if (action === 'status') {
      return ContentService.createTextOutput(JSON.stringify({success: true, message: 'Conectado'}))
...
// ============================================================
// NUEVO: REGISTRAR PROVEEDORES (Directorio de Contactos)
// Independiente de los "proveedores" derivados de Deudas (pestaña Pagar)
// ============================================================
let contactoEliminandoID = null;
...
function renderizarContactosProveedores(filtro = null) {
    const grid = document.getElementById('provGrid');
    const vacio = document.getElementById('provVacio');
    if (!grid || !vacio) return;
    const datos = filtro !== null ? filtro : (DB.proveedoresContacto || []);
    ...
    grid.innerHTML = datos.map(p => {
        const etiquetas = p.etiqueta ? p.etiqueta.split(',').map(t => t.trim()).filter(Boolean) : [];
        const primeraEtiqueta = etiquetas[0] || '';
        const restoEtiquetas = etiquetas.slice(1);
        return `
        <div class="prov-card">
            <div class="prov-card-top">
                <div>${primeraEtiqueta ? `<span class="prov-card-badge">${escapeHtmlProv(primeraEtiqueta)}</span>` : ''}</div>
                <div class="prov-card-quickactions">
                    <button onclick="abrirModalContactoProveedor(${p.id})" title="Editar">✏️</button>
                    <button onclick="eliminarContactoProveedor(${p.id})" title="Eliminar">🗑️</button>
                </div>
            </div>
            <div class="prov-card-body">
                <h4>${escapeHtmlProv(p.nombre)}</h4>
                ${p.alias ? `<div class="prov-card-alias">"${escapeHtmlProv(p.alias)}"</div>` : ''}
                <div class="prov-card-phone">📞 ${escapeHtmlProv(p.numero)}</div>
                ${p.descripcion ? `<div class="prov-card-info">${escapeHtmlProv(p.descripcion)}</div>` : ''}
            </div>
            ${restoEtiquetas.length ? `<div class="prov-tags">${restoEtiquetas.map(t => `<span class="prov-tag">${escapeHtmlProv(t)}</span>`).join('')}</div>` : ''}
            <div class="prov-card-actions">
                <button class="btn-whatsapp" onclick="abrirWhatsAppContacto('${escapeHtmlProv(p.numero)}', '${escapeHtmlProv(p.nombre)}')">💬 Pedir por WhatsApp</button>
                <button class="btn-llamar" onclick="abrirLlamadaContacto('${escapeHtmlProv(p.numero)}')" title="Llamar">📞</button>
            </div>
        </div>
    `;
    }).join('');
...
function buscarContactosProveedores() {
    const q = (document.getElementById('provBuscar').value || '').toLowerCase().trim();
    if (!q) { renderizarContactosProveedores(); return; }
    const filtrados = (DB.proveedoresContacto || []).filter(p => {
        const texto = `${p.nombre} ${p.alias || ''} ${p.etiqueta || ''} ${p.descripcion || ''}`.toLowerCase();
        return texto.includes(q);
    });
    renderizarContactosProveedores(filtrados);
}

function abrirModalContactoProveedor(id = null) {
    document.getElementById('provContactoID').value = '';
    document.getElementById('provContactoNombre').value = '';
    document.getElementById('provContactoNumero').value = '';
    document.getElementById('provContactoEtiqueta').value = '';
    document.getElementById('provContactoAlias').value = '';
    document.getElementById('provContactoDescripcion').value = '';
    document.getElementById('provModalTitulo').textContent = '➕ Agregar Proveedor';

    if (id) {
        const p = (DB.proveedoresContacto || []).find(x => x.id === id);
        if (p) {
            document.getElementById('provModalTitulo').textContent = '✏️ Editar Proveedor';
            document.getElementById('provContactoID').value = p.id;
            document.getElementById('provContactoNombre').value = p.nombre;
            document.getElementById('provContactoNumero').value = p.numero;
            document.getElementById('provContactoEtiqueta').value = p.etiqueta || '';
            document.getElementById('provContactoAlias').value = p.alias || '';
            document.getElementById('provContactoDescripcion').value = p.descripcion || '';
        }
    }
    document.getElementById('modalContactoProveedor').style.display = 'flex';
}

function cerrarModalContactoProveedor() {
    document.getElementById('modalContactoProveedor').style.display = 'none';
}

function guardarContactoProveedor() {
    const nombre = document.getElementById('provContactoNombre').value.trim();
    const numero = document.getElementById('provContactoNumero').value.trim();
    if (!nombre) { alert('⚠️ Debes ingresar el nombre del proveedor'); return; }
    if (!numero) { alert('⚠️ Debes ingresar el número de teléfono'); return; }

    const idStr = document.getElementById('provContactoID').value;
    const datos = {
        nombre,
        numero,
        etiqueta: document.getElementById('provContactoEtiqueta').value.trim(),
        alias: document.getElementById('provContactoAlias').value.trim(),
        descripcion: document.getElementById('provContactoDescripcion').value.trim()
    };
    ...
    if (idStr) {
        ...
        const idx = DB.proveedoresContacto.findIndex(p => p.id === id);
        if (idx !== -1) DB.proveedoresContacto[idx] = { ...DB.proveedoresContacto[idx], ...datos };
    } else {
        const nuevoId = DB.proveedoresContacto.length > 0 ? Math.max(...DB.proveedoresContacto.map(p => p.id)) + 1 : 1;
        DB.proveedoresContacto.push({ id: nuevoId, ...datos });
    }

    guardarEnLocalStorage();
    renderizarContactosProveedores();
    cerrarModalContactoProveedor();
    mostrarIndicadorGuardado();
...
function eliminarContactoProveedor(id) {
    const p = (DB.proveedoresContacto || []).find(x => x.id === id);
    if (!p) return;
    contactoEliminandoID = id;
    document.getElementById('provEliminarNombre').textContent = p.nombre;
    document.getElementById('provEliminarClave').value = '';
    document.getElementById('modalEliminarContactoProveedor').style.display = 'flex';
}

function cerrarModalEliminarContacto() {
    document.getElementById('modalEliminarContactoProveedor').style.display = 'none';
    contactoEliminandoID = null;
}

function confirmarEliminarContactoProveedor() {
    const clave = document.getElementById('provEliminarClave').value;
    if (clave !== CLAVE_ADMIN) { alert('❌ Clave maestra INCORRECTA'); return; }
    if (contactoEliminandoID == null) return;

    DB.proveedoresContacto = (DB.proveedoresContacto || []).filter(p => p.id !== contactoEliminandoID);
    guardarEnLocalStorage();
    renderizarContactosProveedores();
...
function abrirWhatsAppContacto(numero, nombre) {
    const numeroLimpio = numero.replace(/[\s\-\(\)]/g, '');
    const mensaje = `Hola ${nombre}, te contacto desde ${DB.config.NombreNegocio || 'FinanzasPro'}.`;
    window.open(`https://wa.me/${numeroLimpio}?text=${encodeURIComponent(mensaje)}`, '_blank');
}

function abrirLlamadaContacto(numero) {
    const numeroLimpio = numero.replace(/[\s\-\(\)]/g, '');
    window.location.href = `tel:${numeroLimpio}`;
}

function abrirManualUsuario() {
    window.open('manual_uso.html', '_blank');
}

// ============================================================
// NUEVO: BOTÓN DE RETROCESO (DESHACER ÚLTIMA ACCIÓN) - hasta 2 pasos
// Este bloque NO modifica ninguna función existente. Solo "envuelve"
// las funciones que registran movimientos (pagos, cobros, deudas,
// ingresos, etc.) para guardar automáticamente una copia del estado
// justo ANTES de que cada acción se ejecute. Así, si el usuario comete
// un error, puede retroceder hasta 2 pasos y recuperar el estado anterior.
// ============================================================
let undoStack = [];
const UNDO_MAX_PASOS = 2;
...
function guardarSnapshotUndo() {
    try {
        ...
        // Si por algún motivo falla el snapshot, no debe interrumpir la acción original.
    }
}
...
function actualizarBotonDeshacer() {
    ...
    btn.title = undoStack.length > 0
        ? ('Deshacer último cambio (' + undoStack.length + ' disponible' + (undoStack.length > 1 ? 's' : '') + ')')
...
function rerenderizarTodo() {
    ...
    actualizarContexto();
    const ingresoTasaEl = document.getElementById('ingresoTasa');
    if (ingresoTasaEl) ingresoTasaEl.value = DB.config.TasaBCV || 0;
    const notasEl = document.getElementById('notasRapidas');
    if (notasEl) notasEl.value = DB.notas || '';
}

function deshacerUltimaAccion() {
    if (undoStack.length === 0) { alert('ℹ️ No hay ninguna acción reciente para deshacer.'); return; }
    if (!confirm('¿Deshacer la última acción registrada? Se restaurará el estado anterior a ese cambio.')) return;
    ...
    actualizarBotonDeshacer();
    mostrarIndicadorGuardado();
    alert('↩️ Última acción deshecha correctamente.');
}

(function activarSistemaDeDeshacer() {
    // Lista de funciones que representan "acciones" que el usuario puede
    // querer deshacer (pagos, cobros, altas, ediciones y eliminaciones).
    const funcionesAEnvolver = [
        'guardarSaldosIniciales', 'aceptarAjusteSaldo',
        'guardarIngreso', 'guardarEdicionIngreso', 'eliminarIngreso',
        'guardarRecordatorio', 'guardarEdicionRecordatorio', 'toggleRecordatorio', 'eliminarRecordatorio',
        'guardarCuentaPorCobrar', 'confirmarPagoCobro', 'eliminarCuentaPorCobrar',
        'guardarDeuda', 'guardarEdicionDeuda', 'confirmarPagoDeuda', 'eliminarDeuda',
        'guardarEmpleado', 'guardarEdicionEmpleado', 'eliminarEmpleado', 'confirmarPagoNominaIndividual',
        'guardarContactoProveedor', 'confirmarEliminarContactoProveedor'
    ];
    funcionesAEnvolver.forEach(nombre => {
